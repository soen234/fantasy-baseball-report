...
async function main() {
    ...
    printTable(["순위", "이름", "팀", "포지션", "타입"], allPlayers
        .slice(0, 25)
        .map((p) => [
        String(p.rank),
        p.name,
    ...
    for (const pos of batterPositions) {
        ...
        printTable(["순위", "이름", "팀"], group.map((p) => [String(p.rank), p.name, p.team]));
    }
    // 투수
    printSection(`🔥 선발투수 SP (${(positionGroups["SP"] || []).length}명)`);
    if (positionGroups["SP"]) {
        printTable(["순위", "이름", "팀"], positionGroups["SP"].map((p) => [String(p.rank), p.name, p.team]));
    }
    printSection(`💪 구원투수 RP (${(positionGroups["RP"] || []).length}명)`);
    if (positionGroups["RP"]) {
        printTable(["순위", "이름", "팀"], positionGroups["RP"].map((p) => [String(p.rank), p.name, p.team]));
    }
    // 포지션 희소성 분석
    ...
    printTable(["포지션", "Top150 내 인원", "희소성"], scarcity.map(([pos, count]) => [
        pos,
        String(count),
    ...
    console.log(`  타자: ${batters.length}명 (${Math.round((batters.length / 150) * 100)}%)`);
    console.log(`  투수: ${pitchers.length}명 (${Math.round((pitchers.length / 150) * 100)}%)`);
    console.log(`\n  → 로스터 구성: 타자 9슬롯 / 투수 9슬롯 (1:1)`);
    console.log(`  → 랭킹 비율:  타자 ${Math.round((batters.length / 150) * 100)}% / 투수 ${Math.round((pitchers.length / 150) * 100)}%`);
