...
function extractPlayers(data) {
    ...
    for (let i = 0; i < playersObj.count; i++) {
        ...
        const name = playerArr.find((p) => p.name)?.name?.full ?? "Unknown";
        const team = playerArr.find((p) => p.editorial_team_abbr)?.editorial_team_abbr ??
            "?";
        const position = playerArr.find((p) => p.display_position)?.display_position ?? "?";
        const positionType = playerArr.find((p) => p.position_type)?.position_type ?? "?";
        const eligiblePositions = (playerArr.find((p) => p.eligible_positions)?.eligible_positions ?? [])
...
function extractCategories(settingsData) {
    ...
    for (const s of stats) {
        ...
        const direction = stat.sort_order === "1" ? "↑높을수록 좋음" : "↓낮을수록 좋음";
...
function extractRosterPositions(settingsData) {
    const settings = settingsData.fantasy_content.league[1].settings[0];
    return settings.roster_positions.map((rp) => {
...
async function main() {
    ...
    printTable(["포지션", "슬롯 수", "선발/벤치"], roster.map((r) => [
        r.position,
        String(r.count),
        r.starting ? "선발" : "벤치",
    ]));
    // 드래프트 정보
    const draftTime = new Date(Number(settingsData.fantasy_content.league[1].settings[0].draft_time) *
        1000);
    const pickTime = settingsData.fantasy_content.league[1].settings[0].draft_pick_time;
    printSection("드래프트 정보");
    console.log(`  드래프트 시간: ${draftTime.toLocaleString()}`);
    console.log(`  픽 제한 시간: ${pickTime}초`);
    console.log(`  드래프트 방식: ${settingsData.fantasy_content.league[1].settings[0].is_auction_draft === "1" ? "옥션" : "스네이크"}`);
    // 선수 목록 (Top 50)
    printSection("드래프트 선수 랭킹 (Top 50)");
    const playersData = await yahooApi("/league/469.l.18247/players;sort=OR;count=50");
    ...
    console.log(`\n🏏 타자 (${batters.length}명):`);
    printTable(["순위", "이름", "팀", "포지션", "가능 포지션"], batters.map((p) => [
        String(p.rank),
        p.name,
        p.team,
        p.position,
    ...
    printTable(["순위", "이름", "팀", "포지션", "가능 포지션"], pitchers.map((p) => [
        String(p.rank),
        p.name,
        p.team,
        p.position,
