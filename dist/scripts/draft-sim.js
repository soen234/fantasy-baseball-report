...
async function main() {
    // 250명 로드 (넉넉하게)
    const batches = await Promise.all([0, 50, 100, 150, 200].map((start) => yahooApi(`/league/${LEAGUE_KEY}/players;sort=OR;start=${start};count=50`)));
    const allPlayers = [];
    ...
    console.log("※ 앞 픽들이 상위 랭킹 순으로 뽑는다고 가정한 최선/최악 시나리오\n");
    for (let round = 0; round < Math.min(myPicks.length, 15); round++) {
        ...
        const availableEnd = Math.min(availableStart + NUM_TEAMS, allPlayers.length);
        ...
        console.log(`  베스트: ${best.name} (${best.team}, ${best.position}) [${best.rank}위]`);
        console.log(`  워스트: ${worst.name} (${worst.team}, ${worst.position}) [${worst.rank}위]`);
        // 이 범위에서 포지션별 그룹
        const byPos = {};
    ...
    for (const { pos, need, note } of posNeeds) {
        ...
        for (const player of posPlayers.slice(0, Math.min(need + 3, posPlayers.length))) {
            // 이 선수가 내 몇 라운드 픽과 가까운지
            const closestRound = myPicks.findIndex((pick) => pick >= player.rank - 3 && pick <= player.rank + 3);
            const roundStr = closestRound >= 0 ? `→ ${closestRound + 1}R에서 노려볼만` : "";
            console.log(`  ${player.rank}위 ${player.name} (${player.team}) ${roundStr}`);
        }
    }
    // ── 추천 드래프트 플랜 ──
    ...
    printTable(["라운드", "전략", "후보"], plan.map((p) => [p.round, p.strategy, p.candidates]));
    console.log("\n⚠️  핵심 원칙:");
