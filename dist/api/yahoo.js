...
export async function yahooApi(path) {
...
/** 내 리그 목록 가져오기 */
export async function getMyLeagues(season) {
    const gameKey = season ? `mlb.s.${season}` : "mlb";
    const data = await yahooApi(`/users;use_login=1/games;game_keys=${gameKey}/leagues`);
    return data;
}
/** 리그 순위 */
...
/** 리그 스코어보드 */
export async function getScoreboard(leagueKey, week) {
    const weekParam = week ? `;week=${week}` : "";
    return yahooApi(`/league/${leagueKey}/scoreboard${weekParam}`);
}
/** 드래프트 결과 */
export async function getDraftResults(leagueKey) {
...
/** 선수 검색 */
export async function searchPlayers(leagueKey, search) {
    return yahooApi(`/league/${leagueKey}/players;search=${encodeURIComponent(search)}`);
}
/** 리그 트랜잭션 */
export async function getTransactions(leagueKey) {
...
/** FA (Free Agent) 목록 */
export async function getFreeAgents(leagueKey, position) {
