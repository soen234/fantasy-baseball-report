...
function extractPlayers(data: any, startRank: number): PlayerInfo[] {
  ...
  for (let i = 0; i < playersObj.count; i++) {
    ...
    players.push({
      ...
      eligiblePositions: (
        arr.find((p: any) => p.eligible_positions)?.eligible_positions ?? []
      )
...
function getMyPicks(pick: number, teams: number, rounds: number): number[] {
  const picks: number[] = [];
...
async function main() {
  ...
  const batches = await Promise.all(
    [0, 50, 100, 150, 200, 250, 300].map((s) =>
      yahooApi(`/league/${LEAGUE_KEY}/players;sort=OR;start=${s};count=50`),
    ),
  );
  ...
  for (const pos of scarcityOrder) {
    const mainPosPlayers = allPlayers.filter(
  ...
  const globalDrafted = new Set<number>(); // 다른 팀이 뽑은 선수 rank
  const myDrafted = new Set<number>(); // 내가 뽑은 선수 rank

  // 전체 드래프트를 시뮬레이션
  let nextAvailIdx = 0; // allPlayers에서 다음으로 뽑힐 인덱스
  for (
    let overallPick = 1;
    overallPick <= TOTAL_ROUNDS * NUM_TEAMS;
    overallPick++
  ) {
    ...
    if (myRound === -1) {
      ...
      while (
        nextAvailIdx < allPlayers.length &&
  ...
  const globalDrafted2 = new Set<number>();
  const myDrafted2 = new Set<number>();
  let nextIdx = 0;

  /** 선수가 채울 수 있는 니즈 포지션 반환 */
  function getMatchingNeed(
    p: PlayerInfo,
    currentNeeds: Record<string, number>,
  ): string | null {
    // 정확한 포지션 매칭 우선
    for (const ep of p.eligiblePositions) {
      if (ep !== "Util" && currentNeeds[ep] && currentNeeds[ep] > 0) return ep;
    }
    // Util 슬롯
    if (p.positionType === "B" && currentNeeds["Util"] > 0) return "Util";
    // P 슬롯 (SP/RP 둘 다 가능)
    if (p.positionType === "P" && currentNeeds["P"] > 0) return "P";
    // BN
    if (currentNeeds["BN"] > 0) return "BN";
    return null;
  }

  /** 포지션 희소성 점수 (낮을수록 희소) */
  function scarcityScore(p: PlayerInfo, available: PlayerInfo[]): number {
  ...
  function getUrgentPositions(
    ...
    const urgent: [string, number, number][] = []; // [pos, supply, urgencyScore]
    const positionsToCheck = ["C", "1B", "2B", "3B", "SS", "OF", "SP", "RP"];
    for (const pos of positionsToCheck) {
      const need = currentNeeds[pos] ?? 0;
      if (need <= 0) continue;
      const supply = available.filter(
        (p) =>
          p.eligiblePositions.includes(pos) ||
          p.position.split(",")[0].trim() === pos,
      ).length;
  ...
  for (
    ...
    const available = allPlayers.filter(
      (p) => !globalDrafted2.has(p.rank) && !myDrafted2.has(p.rank),
    );
    ...
    const scored = candidates.map((p) => {
      ...
      // - SP는 4~6R에 시작
      if (myRound <= 4) {
        if (mainPos === "RP") score -= 50; // 초반 RP 페널티
        if (p.positionType === "B") score += 15; // 초반 타자 보너스
      } else if (myRound <= 6) {
        if (mainPos === "SP") score += 10;
      }
      ...
      if (myRound >= 1) {
        ...
        if (needs["3B"] > 0 && p.eligiblePositions.includes("3B")) {
          if (myRound <= 4) score += 50 * scarceBoost;
          else if (myRound <= 7) score += 25 * scarceBoost;
        }
        if (needs["C"] > 0 && p.eligiblePositions.includes("C")) {
          if (myRound <= 4) score += 45 * scarceBoost;
          else if (myRound <= 7) score += 20 * scarceBoost;
        }
      }
    ...
    if (recommended.length > 0) {
      ...
      recommended.forEach(({ player: p, matchedNeed }, idx) => {
        ...
        w(
          `| ${star} | ${p.rank}위 | **${p.name}** | ${p.team} | ${p.position} | ${slot} | ${note} |`,
        );
      });
    } else {
      w("_가용 선수 없음_");
  ...
  for (const pos of tierPositions) {
    ...
    posPlayers.forEach((p, idx) => {
      let tier = "";
      if (idx < 2) tier = "🥇 엘리트";
      else if (idx < 5) tier = "🥈 상위";
      else if (idx < 10) tier = "🥉 중위";
      else tier = "보통";
